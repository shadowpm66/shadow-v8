...
def _smoke_paths() -> dict:
    root = Path("runtime") / "smoke" / "bybit_private_status_surface"
    return {
        **config_module.PATHS,
        "dashboard": root,
...
def main() -> None:
    ...
    private_status = {
        "status": "READY_FOR_READ_ONLY_PRIVATE_VALIDATION",
        "private_validation_status": "SIGNED_PREVIEW_READY",
        ...
        "next_action": "run_execute_private_request_on_ec2",
    }
    ...
    assert_true(
        status["bybit_private_validation"]["status"] == "READY_FOR_READ_ONLY_PRIVATE_VALIDATION",
        "Status JSON should include private validation state",
    )

    text = CommandProcessor(bot=FakeBot())._status_text({"entries_paused": False})
    assert_true("Bybit private: READY_FOR_READ_ONLY_PRIVATE_VALIDATION" in text, "Telegram status should include Bybit private state")
    assert_true("Private next: run_execute_private_request_on_ec2" in text, "Telegram status should include private next action")
    ...
    assert_true("Bybit private" in html, "Dashboard should include Bybit private label")
    assert_true("READY_FOR_READ_ONLY_PRIVATE_VALIDATION" in html, "Dashboard should render private validation state")
    assert_true("run_execute_private_request_on_ec2" in html, "Dashboard should render private next action")
    ...
    print("Bybit private status surface smoke complete")
    print("ok=True")
    print(text)
