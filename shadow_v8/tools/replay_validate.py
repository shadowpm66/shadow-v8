...
def summary_row(result: dict[str, Any]) -> dict[str, Any]:
    ...
    top_stage_block_detail = None
    for item in gate.get("top_stage_block_details") or []:
        name = item.get("name")
        if top_stage_block_reason and str(name).startswith(f"{top_stage_block_reason}:"):
            top_stage_block_detail = name
            break
    if top_stage_block_detail is None:
        top_stage_block_detail = (gate.get("top_stage_block_details") or [{}])[0].get("name")
    ...
    return {
        ...
        "top_stage_block_detail": top_stage_block_detail,
