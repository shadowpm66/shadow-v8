...
def check_replay_fixture() -> dict:
    ...
    assert_true(result["schema_version"] == "1.5.23", "Replay result should include schema_version")
    ...
    assert_true("exit_analytics" in result, "Replay result should include exit analytics")
    ...
    assert_true("shift_progress_bucket" in pivot, "Pivot confirmation should include shift progress bucket")
    ...
    assert_true("stage" in gate, "Trade gate should include stage diagnostics")
    assert_true("long_weekly_compatible" in gate["stage"], "Stage diagnostics should include long weekly compatibility")
    assert_true("long_daily_compatible" in gate["stage"], "Stage diagnostics should include long daily compatibility")
    ...
    assert_true(
        "pivot_shift_progress_buckets_by_status" in analytics,
        "Gate analytics should include pivot shift buckets by gate status",
    )
    assert_true("watch_readiness_buckets" in analytics, "Gate analytics should include watch readiness buckets")
    assert_true("near_entry_watch_samples" in analytics, "Gate analytics should include near-entry watch samples")
    assert_true(
        "countertrend_reclaim_candidates" in analytics,
        "Gate analytics should include countertrend reclaim candidates",
    )
    assert_true(
        "countertrend_reclaim_samples" in analytics,
        "Gate analytics should include countertrend reclaim samples",
    )
